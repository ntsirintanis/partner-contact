...
class TestPartnerRelation(PartnerRelationCase):
    ...
    def test_create_with_active_id(self):
        ...
        relation = self.relation_all_model.with_context(
            ...
            active_ids=self.partner_company_test.ids).create({
    ...
    def test_on_change_type_selection(self):
        ...
        self.assertTrue(
            ('category_id', 'in', [self.category_ngo.id]) in
            domain['this_partner_id'])
        self.assertTrue(
            ('category_id', 'in', [self.category_volunteer.id]) in
    ...
    def test_compute_domains(self):
        ...
        self.assertIn(
            ...
            json.loads(relation.type_selection_id_domain))
        self.assertIn(
            [u'partner_category_this', u'in',
             self.partner_ngo_test.category_id.ids],
            json.loads(relation.type_selection_id_domain))
        # Selection of other_partner_id must be for persons that
        # are volunteers.
        self.assertIn(
            [u'is_company', u'=', False],
    ...
    def test_write(self):
        ...
        self.assertEqual(
            relation.this_partner_id.id,
            partner_extra_person.id)
        self.assertEqual(
            relation.type_selection_id.id,
            self.selection_person_is_director.id)
        self.assertEqual(
